...
class ChunkCloudDeleter:
    ...
    def delete_manifest_chunks(
        ...
        *,
        ...
        """
        Takes a manifest, iterates through its chunks, and deletes them from the cloud providers.
        ...
        for chunk in sorted(manifest.get("chunks", []), key=lambda c: c.get("index", 0)):
            ...
            try:
                ...
                print(f"SUCCESSFULLY deleted chunk {chunk['name']} from {source} (ID: {file_id})")
                
                # Remove the cloud_file_id from the chunk to mark it as deleted
                del chunk["cloud_file_id"]
                ...
                raise HTTPException(
                    status_code=502,
                    detail=(
                        f"Failed deleting chunk {chunk['name']} from {source}: "
                        f"{provider_error}"
                    ),
                )
